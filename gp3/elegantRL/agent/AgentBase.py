...
class AgentBase:
    ...
    def __init__(self, net_dims: List[int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        self.act = self.act_target = act_class(net_dims, state_dim, action_dim, activation).to(self.device)
        self.cri = self.cri_target = cri_class(net_dims, state_dim, action_dim, activation).to(self.device) \
        ...
        self.act_optimizer = optimizer(self.act.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)
        self.cri_optimizer = optimizer(self.cri.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay) \
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        if_random: uses random action for warm-up exploration
    ...
    def save_or_load_agent(self, cwd: str, if_save: bool):
        ...
        cwd: Current Working Directory. ElegantRL saves training files in cwd.
