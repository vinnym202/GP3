...
class Evaluator:
    ...
    def evaluate_and_save(self, actor: torch.nn, steps: int, exp_r: float, logging_tuple: tuple):
        ...
        print(f"{self.agent_id:<3} {self.total_step:8.2e}{train_time:7.0f} |"
        ...
        if if_save:  # save checkpoint with the highest episode return
            if self.if_over_write:
                actor_path = f"{self.cwd}/actor_model_avgR.pth"
            else:
                actor_path = f"{self.cwd}/actor_model_avgR_{self.total_step:012}_{self.max_r:09.3f}.pth"
            torch.save(actor, actor_path)  # save policy network in *.pth
            
        if self.save_counter == self.save_gap:
            self.save_counter = 0
            if self.if_over_write:
                actor_path = f"{self.cwd}/actor_model.pth"
            else:
                actor_path = f"{self.cwd}/actor_model_{self.total_step:012}.pth"
            torch.save(actor, actor_path)  # save policy network in *.pth

    def save_or_load_recorder(self, if_save: bool):
        if if_save:
...
def get_cumulative_rewards_and_steps(env, actor, if_render: bool = False) -> Tuple[float, int]:
    ...
    for steps in range(max_step):
        ...
        print("| get_rewards_and_step: WARNING. max_step > MAXIMUM")
